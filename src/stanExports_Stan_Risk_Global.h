...
public:
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        if (!(context__.contains_r("global_theta")))
            stan::lang::rethrow_located(std::runtime_error(std::string("Variable global_theta missing")), current_statement_begin__, prog_reader__());
        vals_r__ = context__.vals_r("global_theta");
        pos__ = 0U;
        context__.validate_dims("parameter initialization", "global_theta", "double", context__.to_vec());
        double global_theta(0);
        global_theta = vals_r__[pos__++];
        try {
            writer__.scalar_lb_unconstrain(0, global_theta);
        } catch (const std::exception& e) {
            stan::lang::rethrow_located(std::runtime_error(std::string("Error transforming variable global_theta: ") + e.what()), current_statement_begin__, prog_reader__());
        }
